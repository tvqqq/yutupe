...
- Deep-history feed dùng `playlistItems.contentDetails.videoPublishedAt`, không dùng `playlistItems.snippet.publishedAt` vì trường đó chỉ là ngày item được thêm vào playlist. Cache sanitizer v3 xóa riêng video cache cũ một lần và queue enrichment để loại timestamp sai khỏi AI Recommend; groups, channels và watched/hidden state được giữ nguyên.
- Mọi video card có thumbnail URL: ưu tiên URL từ API/RSS/DOM, sau đó fallback deterministic về `https://i.ytimg.com/vi/<videoId>/hqdefault.jpg`; lỗi tải URL nguồn cũng đổi sang fallback này.
