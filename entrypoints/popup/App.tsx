...
function ChannelsPage({ state, act }: { state: AppState; act: (message: AppMessage) => Promise<unknown> }) {
  ...
  const isMissingFeed = (channel: Channel) => !channel.lastPublishedAt && channel.enrichment?.status !== 'loading' && channel.enrichment?.status !== 'error';
  const allMissingFeedChannels = useMemo(() => state.channels.filter(isMissingFeed), [state.channels]);
  ...
  const missingFeedChannels = channels.filter(isMissingFeed);
  const loadedFeedChannels = channels.filter((channel) => !isMissingFeed(channel));
  ...
  const renderChannel = (channel: Channel) => <ChannelRow key={channel.id} channel={channel} state={state} act={act} selected={selected.has(channel.id)} onSelect={() => setSelected((current) => { const next = new Set(current); if (next.has(channel.id)) next.delete(channel.id); else next.add(channel.id); return next; })} />;
  return <section className="page">
    ...
    {!channels.length ? <EmptyState title="Chưa tìm thấy channel" detail="Kết nối Google rồi bấm Sync YouTube, hoặc cuộn trang YouTube để thu thập local." /> : <div className="channel-sections">
      {missingFeedChannels.length > 0 && <section className="missing-feed-section"><header><div><span className="missing-feed-icon"><CircleAlert size={17} /></span><div><h2>Chưa tải Feed</h2><p>Các channel chưa xác định được video mới nhất.</p></div></div><div><span className="missing-feed-count">{missingFeedChannels.length}{query ? `/${allMissingFeedChannels.length}` : ''} channels</span><button className="primary small" disabled={Boolean(busy) || !allMissingFeedChannels.length} onClick={() => void runAction('missing-feed', { type: 'REFRESH_YOUTUBE_FEED', payload: { groupId: null, perChannel: 1, channelIds: allMissingFeedChannels.map((channel) => channel.id) } }, `Đã refresh riêng ${allMissingFeedChannels.length} channel chưa có Feed.`)}><RefreshCw className={busy === 'missing-feed' ? 'spin' : ''} size={14} />{busy === 'missing-feed' ? 'Đang refresh…' : 'Refresh các channel này'}</button></div></header><div className="channel-list">{missingFeedChannels.map(renderChannel)}</div></section>}
      {loadedFeedChannels.length > 0 && <section className="loaded-feed-section"><header><div><h2>Channels đã có dữ liệu Feed</h2><p>Đã có video mới nhất hoặc đang được background cập nhật.</p></div><span>{loadedFeedChannels.length} channels</span></header><div className="channel-list">{loadedFeedChannels.map(renderChannel)}</div></section>}
    </div>}
  </section>;
}
